"""Regenerate figures, compile and validate the six-to-eight-page report body."""
import hashlib
import json
import os
...
analysis = subprocess.run([sys.executable, str(ROOT/"scripts/analyze.py")],
...
assert any("RelationtoPacingtheFrontier" in t for t in normalized)
...
validation = dict(body_pages=body_pages, narrative_pages=narrative_pages,
                  ...
                  author="Alejandro Garibotti", model_calls=0, configurations=192,
                  verified="Automated checks only; independent author review pending",
                  source_freeze_sha256=hashlib.sha256((ROOT/"configs/freeze.json").read_bytes()).hexdigest(),
